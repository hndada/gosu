...
func NewFrames(frames []*ebiten.Image, period time.Duration) Frames {
	return Frames{
		Frames:    frames,
		StartTime: times.Now(),
		Period:    period,
...
func NewAnimationFromFile(fsys fs.FS, name string, period time.Duration) Animation {
	imgs := NewFramesFromFile(fsys, name)
...
func NewFramesFromFile(fsys fs.FS, name string) []*ebiten.Image {
