...
import (
	...
	"github.com/hndada/gosu/game/scene"
	"image"
	"image/color"
	"io/ioutil"
)
...
func (s *SceneMania) Update(state *game.State) error {
	var endTime = 2.40 * 1000 // float64(s.C.Notes[len(s.C.Notes)-1].Time)
	...
	if s.Time > endTime {
		state.NextScene = &scene.Result{}
		state.TransCountdown = 99 // todo: set maxCount
	}
	return nil
}
