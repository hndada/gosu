...
// Alternative name: ImageSeq, Images,
// Slideshow, Footage, Clip
...
func NewAnimation(frames []*ebiten.Image, period time.Duration) Animation {
	return Animation{
		Frames:    frames,
		Box:       NewBox(frames[0]),
		StartTime: times.Now(),
...
func NewAnimationFromFile(fsys fs.FS, name string, period time.Duration) Animation {
	...
	return NewAnimation(imgs, period)
}
...
func NewFramesImagesFromFile(fsys fs.FS, name string) []*ebiten.Image {
	base := strings.TrimSuffix(name, filepath.Ext(name))
	paths := framePaths(fsys, base)
	if len(paths) == 0 {
		one := NewImageFromFile(fsys, name)
		return []*ebiten.Image{one.Image}
	}

	fs := make([]*ebiten.Image, len(paths))
